...
# Name		: fcn.dir
# Author	: VKS
# Date		: 12/22/17,1/24/18,2/1,8/16/19,8/19,2/4/20,12/25,12/20/22
# Args		: none
...
# -----------------------------------------------------------------
	...
	z <- paste0(z, "\\root.txt")
	if (file.exists(z)) z <- readLines(z) else z <- "<EXTERNAL>"
	z
}
...
# Name		: mk.beta
# Author	: VKS
# Date		: 12/27/16,1/23/18,4/4,4/25,12/20/22
# Args		: x = a single YYYYMM
#		: y = a string vector, the elements of which are:
...
# Name		: parameters
# Author	: VKS
# Date		: 3/15/17,12/15,12/20/22
# Args		: x = parameter type
# Output	: returns full path to relevant parameters file
# -----------------------------------------------------------------
	parameters.ex.file(dir.parameters("parameters"), paste0(x, ".txt"))
}
parameters.ex.file <- function(x, y) {
# -----------------------------------------------------------------
# Name		: parameters.ex.file
# Author	: VKS
# Date		: 12/20/22
# Args		: x = folder paths
#		: y = file names of the same length as <x>
# Output	: path to function source file
# -----------------------------------------------------------------
#
# HANDLE EXTERNALS
...
# CREATE RESULT
	z <- paste0(x, "\\", y)
	w <- file.exists(z)
	if (any(!w)) err.raise(z[!w], T, "WARNING: The following files do not exist")
#
# RETURN RESULT
	z
}
...
# Author	: VKS
# Date		: 10/29/18,11/26,12/20,1/25/19,10/22/22,12/20
...
# BENCHMARK TOTAL RETURN INDEX
	y <- map.rname(mat.read(parameters.ex.file(dir.parameters("csv"), "IndexReturns-Daily.csv")), dimnames(x)[[1]])[, y]
	x[, "Benchmark"] <- y
#
...
#
# CREATE RESULT
	z <- x - y * rowSums(x * y, na.rm = T)/nonneg(rowSums(y^2, na.rm = T))
#
# RETURN RESULT
	z
}
...
# Name		: qa.mat.read
# Author	: VKS
# Date		: 10/8/18,10/12,11/26,8/16/19,12/15/22
# Args		: x = remote file on an ftp site (e.g. "/ftpdata/mystuff/foo.txt")
#		: y = local folder (e.g. "C:\\temp")
#		: n = ftp site (defaults to standard)
#		: w = user id (defaults to standard)
...
# Name		: record.kill
# Author	: VKS
# Date		: 2/11/20,12/17,12/25,12/20/22
# Args		: x = report name
#		: y = file name
...
#		: 10/9,10/22,11/2,11/9/20,12/2/22
# Args		: x = the YYYYMM for which you want data (known 26 days later)
#		: y = a string vector of factors to be computed,
#		:	the last element of which is the type of fund used.
#		: n = any of StockFlows/China/Japan/CSI300/Energy
...
# CREATE RESULT
	...
	z <- c(paste0("\t", n, " (", paste(c("FundId", w, r, x), collapse = ", "), ")"), z)
	z <- c("insert into", z)
	z <- c(sql.delete(n, sql.in("FundId", sql.tbl("FundId", "#FLO", h))), "", z)
#
# RETURN RESULT
	z
}
...
# Name		: strat.dir
# Author	: VKS
# Date		: 5/16/17,8/28,12/15,12/20/22
...
# Name		: stratrets.data
# Author	: VKS
# Date		: 10/26/22,11/14,12/20/22,12/26
# Args		: x = strategy
#		: y = variable being back-tested (e.g. FloPct/AIS)
...
# INDICATOR PATH
	if (is.na(h[x, "path"])) {
		z <- mat.read(parameters("classif-strat-multi"), "\t", NULL)
...
# Name		: stratrets.path
# Author	: VKS
# Date		: 11/15/22,11/16,11/17,11/22,12/20,12/21,12/22
# Args		: x = indicator type (e.g. Ctry/FX/SectorUK/Rgn/FI)
#		: y = FundType (e.g. E/B)
#		: n = filter (e.g. Aggregate, Act, SRI. etc.)
#		: w = item (e.g. Flow/AssetsStart/Result)
#		: h = variant (e.g. CB/SG/CBSG)
...
# SPECIAL OPERATIONS
	...
	} else if (x == "Rgn" & y == "E" & n == "Act" & w == "Result" & h == "SG") {
		z <- strat.path("MultiAsset-Rgn", "daily")
	} else if (x == "FI" & y == "B" & n == "Aggregate" & w == "Result" & h == "SG") {
	...
	} else if (x == "Ctry" & y == "B" & n == "Aggregate" & w == "Result" & h == "CB") {
...
# CREATE RESULT
	if (is.null(z)) {
		...
		z <- parameters.ex.file(z, paste0("oneDayFloMo", h, y, n, w, ".csv"))
	}
#
# RETURN RESULT
	z
...
# Author	: VKS
# Date		: 10/25/22,10/26,10/27,12/20
...
# -----------------------------------------------------------------
	if (x == "Ctry") {
		...
		x <- parameters.ex.file(x, c("OfclMsciTotRetIdx.csv", "pseudoReturns.csv"))
		...
		if (any(dimnames(y)[[2]] == x)) {
			...
			z <- paste0(fcn.dir(), "\\New Model Concept\\Sector", x, "\\FloMo\\csv")
...
# Name		: txt.words
# Author	: VKS
# Date		: 6/21/18,11/27,12/25/20,12/20/22
